...
# pip install python-telegram-bot python-a2s beautifulsoup4 requests flask pillow
# =============================================
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .info-item { background: #0d0d14; border-radius: 10px; padding: 14px; }
    .info-label { font-size: 0.7em; color: #555; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 4px; }
    .info-value { font-size: 1.2em; font-weight: 700; color: #fff; }
    .info-value.orange { color: #f7931e; }
    .info-value.green  { color: #2ecc71; }

    .bar-wrap { background: #0d0d14; border-radius: 10px; padding: 14px; margin-top: 12px; }
    .bar-label { font-size: 0.75em; color: #555; margin-bottom: 8px; }
    .bar-track { background: #1a1a2e; border-radius: 20px; height: 8px; overflow: hidden; }
    .bar-fill {
      height: 100%;
    ...
    .player-row {
      ...
      padding: 10px 0; border-bottom: 1px solid #1a1a1a; font-size: 0.9em;
    }
    ...
    .stat-item { background: #0d0d14; border-radius: 10px; padding: 14px; display: flex; align-items: center; gap: 10px; }
    .stat-emoji { font-size: 1.4em; }
    .stat-info .stat-val { font-size: 1.1em; font-weight: 700; }
    .stat-info .stat-name { font-size: 0.7em; color: #555; }
    .stat-change { font-size: 0.7em; color: #2ecc71; margin-left: 4px; }
    ...
    .refresh-info { text-align: center; color: #333; font-size: 0.75em; margin-top: 16px; }
...
def top_keyboard(mode):
    sw = (
        InlineKeyboardButton("⏱ Vaqt", callback_data="show_top_time")
        if mode == "kills" else
        InlineKeyboardButton("💀 Kill", callback_data="show_top_kills")
    )
    rc = "refresh_top_kills" if mode == "kills" else "refresh_top_time"
    return InlineKeyboardMarkup([[
        sw,
        InlineKeyboardButton("🔄 Yangilash", callback_data=rc),
...
# ──────────────────────────────────────────────
# YORDAMCHI: Eski xabarni o'chirib yangi rasm yuborish
# ──────────────────────────────────────────────
async def _send_photo(update_or_query, photo, keyboard, is_callback=False):
    """Callback yoki message'dan rasm yuborish"""
    if is_callback:
        query = update_or_query
        # Eski xabarni o'chirib yangi rasm yuborish
        try:
            await query.message.delete()
        except Exception:
            pass
        await query.message.chat.send_photo(photo=photo, reply_markup=keyboard)
    else:
        await update_or_query.message.reply_photo(photo=photo, reply_markup=keyboard)
...
async def cmd_start(update, context):
    await update.message.reply_html(
        ...
        "📌 <b>Buyruqlar:</b>\n"
        "🔹 <code>Online</code> — Server holati (rasm)\n"
        "🔹 /status — Server holati (rasm)\n"
        "🔹 /top — TOP o'yinchilar (rasm)\n"
        "🔹 /sayt — Sayt statistikasi (rasm)\n"
        "🔹 /help — Yordam"
    )
...
async def cmd_status(update, context):
    ...
    await msg.delete()
    photo = make_status_image(result, CS_HOST, CS_PORT)
    await update.message.reply_photo(photo=photo, reply_markup=status_keyboard())

async def cmd_top(update, context):
    msg = await update.message.reply_html("⏳ <b>TOP yuklanmoqda...</b>")
    result = await asyncio.get_event_loop().run_in_executor(None, query_server)
    await msg.delete()
    photo = make_top_image(result, "kills", CS_HOST, CS_PORT)
    await update.message.reply_photo(photo=photo, reply_markup=top_keyboard("kills"))
...
async def cmd_sayt(update, context):
    ...
    await msg.delete()
    photo = make_site_stats_image(sr, st, CS_HOST, CS_PORT)
    await update.message.reply_photo(photo=photo, reply_markup=site_stats_keyboard())

async def on_online(update, context):
    """'Online' so'zi yozilganda status rasmi yuborish"""
...
async def on_callback(update, context):
    ...
    if data in ("refresh_status", "back_to_status"):
        ...
        photo  = make_status_image(result, CS_HOST, CS_PORT)
        await _send_photo(query, photo, status_keyboard(), is_callback=True)
    ...
    # ── TOP KILLS ──
        ...
        result = await loop.run_in_executor(None, query_server)
        photo  = make_top_image(result, "kills", CS_HOST, CS_PORT)
        await _send_photo(query, photo, top_keyboard("kills"), is_callback=True)

    # ── TOP VAQT ──
    elif data in ("show_top_time", "refresh_top_time"):
        await query.answer("🔄 Yangilanmoqda..." if "refresh" in data else "")
        result = await loop.run_in_executor(None, query_server)
        photo  = make_top_image(result, "time", CS_HOST, CS_PORT)
        await _send_photo(query, photo, top_keyboard("time"), is_callback=True)
    ...
    # ── SAYT STATS ──
        ...
        st = await loop.run_in_executor(None, query_site_stats)
        photo = make_site_stats_image(sr, st, CS_HOST, CS_PORT)
        await _send_photo(query, photo, site_stats_keyboard(), is_callback=True)
